    ...
    def __init__(self, emp_username):
        ...
        for emp in range(len(employee_data)):
            if employee_data[emp].get("username").__eq__(emp_username):
                ...
                break
        if found != -1:
            self.emp_name = employee_data[found].get('emp_name')
            self.emp_id = employee_data[found].get('emp_id')
            self.age = employee_data[found].get("age")
            self.salary = employee_data[found].get("salary")
            self.role = employee_data[found].get("role")
            self.dept = employee_data[found].get("dept")
